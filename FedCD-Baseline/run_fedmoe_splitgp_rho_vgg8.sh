...
run_tag="fedmoe_splitgp_rho_${date_str}_${time_str}"
queue_root="$SCRIPT_DIR/batch_runs/fedmoe_splitgp_rho_vgg8/date_${date_str}/time_${time_str}"
...
wait_for_batch() {
  ...
  for i in "${!batch_pids[@]}"; do
    ...
    if [[ $exit_code -eq 0 ]]; then
      status="ok"
      echo "[DONE ${batch_idxs[$i]}/$total] dataset=${batch_datasets[$i]} algo=FedMoE seed=$SEED"
    else
      status="failed"
      echo "[FAIL ${batch_idxs[$i]}/$total] dataset=${batch_datasets[$i]} algo=FedMoE seed=$SEED exit_code=$exit_code"
      tail -n 40 "${batch_logs[$i]}" || true
    fi
    printf '%s\n' "${batch_idxs[$i]},${total},${batch_dataset_bases[$i]},FedMoE,${batch_rhos[$i]},${NUM_CLIENTS},${SEED},${batch_datasets[$i]},${status},${exit_code},${batch_starts[$i]},${end_utc},${batch_logs[$i]}" >> "$status_csv"
    echo
  done
  batch_pids=()
  batch_idxs=()
...
for dataset_base in "${datasets[@]}"; do
  ...
  for rho in "${rhos[@]}"; do
    ...
    idx=$((idx + 1))
    safe_rho="${rho//./p}"
    goal="FedMoE_splitgp_rho${rho}_nc${NUM_CLIENTS}_${run_tag}_seed${SEED}"
    run_log="$run_log_dir/${idx}_${dataset_base}_FedMoE_rho${safe_rho}_nc${NUM_CLIENTS}_seed${SEED}.log"
    start_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

    echo "=========================================================="
    echo "[START $idx/$total] dataset=$dataset algo=FedMoE seed=$SEED"
    echo "[LOG] $run_log"
    echo "=========================================================="
...
echo "[INFO] FedMoE SplitGP rho queue finished."

...
class clientpFedMe(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.local_params = self._clone_param_list(self.model.parameters())
        self.personalized_params = self._clone_param_list(self.model.parameters())
        ...
        for epoch in range(max_local_epochs):  # local update
            for x, y in trainloader:
                ...
                for new_param, localweight in zip(self.personalized_params, self.local_params):
                    ...
                    updated_local = localweight.data - self.lamda * self.learning_rate * (localweight.data - new_param.data)
                    updated_local_params.append(updated_local.detach().cpu().clone())
                self.local_params = updated_local_params
    ...
    def train_metrics_personalized(self):
        ...
        with torch.no_grad():
            for x, y in trainloader:
                ...
                lm = torch.cat([p.detach().to(self.device).view(-1) for p in self.local_params], dim=0)
                pm = torch.cat([p.detach().to(self.device).view(-1) for p in self.personalized_params], dim=0)
                loss += 0.5 * self.lamda * torch.norm(lm-pm, p=2)

                train_acc += (torch.sum(torch.argmax(output, dim=1) == y)).item()
                
                train_num += y.shape[0]
                losses += loss.detach().item() * y.shape[0]

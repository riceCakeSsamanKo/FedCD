...
WINDOW_NAME="${2:-VGG8-C10-NC50}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_SCRIPT="$SCRIPT_DIR/run_requested_baselines_vgg8_cifar10_nc50.sh"
...
RUN_CMD="cd \"$SCRIPT_DIR\" && bash \"$RUN_SCRIPT\"; EXIT_CODE=\$?; echo; echo \"[EXIT] requested baseline NC50 queue finished with code \$EXIT_CODE\"; exec bash"

...
CLUSTER_THRESHOLD="${CLUSTER_THRESHOLD:-0.1}"
ADAPTIVE_THRESHOLD="${ADAPTIVE_THRESHOLD:-true}"
CLUSTER_PERIOD="${CLUSTER_PERIOD:-2}"
PM_PERIOD="${PM_PERIOD:-1}"
GLOBAL_PERIOD="${GLOBAL_PERIOD:-1}"
CLUSTER_SAMPLE_SIZE="${CLUSTER_SAMPLE_SIZE:-512}"
FEDCD_NC_WEIGHT="${FEDCD_NC_WEIGHT:-0.0}"
FEDCD_NC_TARGET_CORR="${FEDCD_NC_TARGET_CORR:--0.1}"
THRESHOLD_STEP="${THRESHOLD_STEP:-0.01}"
THRESHOLD_STEP_MAX="${THRESHOLD_STEP_MAX:-0.1}"
THRESHOLD_DECAY="${THRESHOLD_DECAY:-0.9}"
ACT_WINDOW_SIZE="${ACT_WINDOW_SIZE:-5}"
ACT_MIN_SLOPE="${ACT_MIN_SLOPE:-0.0002}"
THRESHOLD_INC_RATE="${THRESHOLD_INC_RATE:-1.3}"
THRESHOLD_DEC_RATE="${THRESHOLD_DEC_RATE:-0.5}"
THRESHOLD_MAX="${THRESHOLD_MAX:-0.95}"
EMA_ALPHA="${EMA_ALPHA:-0.3}"
TOLERANCE_RATIO="${TOLERANCE_RATIO:-0.4}"
...
FEDCD_PM_TEACHER_COMPETENCE_FILTER="${FEDCD_PM_TEACHER_COMPETENCE_FILTER:-false}"
FEDCD_PM_TEACHER_COMPETENCE_REL_WEIGHT="${FEDCD_PM_TEACHER_COMPETENCE_REL_WEIGHT:-0.5}"
FEDCD_PM_TEACHER_COMPETENCE_SUPPORT_WEIGHT="${FEDCD_PM_TEACHER_COMPETENCE_SUPPORT_WEIGHT:-0.2}"
FEDCD_PM_TEACHER_COMPETENCE_PROTO_WEIGHT="${FEDCD_PM_TEACHER_COMPETENCE_PROTO_WEIGHT:-0.3}"
FEDCD_PM_TEACHER_COMPETENCE_MIN="${FEDCD_PM_TEACHER_COMPETENCE_MIN:-0.35}"
FEDCD_PM_TEACHER_CLUSTER_DIST_WEIGHTING="${FEDCD_PM_TEACHER_CLUSTER_DIST_WEIGHTING:-false}"
FEDCD_PM_TEACHER_CLUSTER_DIST_TAU="${FEDCD_PM_TEACHER_CLUSTER_DIST_TAU:-1.0}"
FEDCD_PM_TEACHER_CLUSTER_DIST_METRIC="${FEDCD_PM_TEACHER_CLUSTER_DIST_METRIC:-mahalanobis}"
FEDCD_PM_TEACHER_DATAFREE_ENABLE="${FEDCD_PM_TEACHER_DATAFREE_ENABLE:-false}"
FEDCD_PM_TEACHER_DATAFREE_BATCHES="${FEDCD_PM_TEACHER_DATAFREE_BATCHES:-32}"
FEDCD_PM_TEACHER_DATAFREE_STEPS="${FEDCD_PM_TEACHER_DATAFREE_STEPS:-20}"
FEDCD_PM_TEACHER_DATAFREE_LR="${FEDCD_PM_TEACHER_DATAFREE_LR:-0.05}"
FEDCD_PM_TEACHER_DATAFREE_NOISE_SCALE="${FEDCD_PM_TEACHER_DATAFREE_NOISE_SCALE:-0.25}"
FEDCD_PM_TEACHER_DATAFREE_ENTROPY_WEIGHT="${FEDCD_PM_TEACHER_DATAFREE_ENTROPY_WEIGHT:-1.0}"
FEDCD_PM_TEACHER_DATAFREE_DIVERSITY_WEIGHT="${FEDCD_PM_TEACHER_DATAFREE_DIVERSITY_WEIGHT:-0.5}"
FEDCD_PM_TEACHER_DATAFREE_L2_WEIGHT="${FEDCD_PM_TEACHER_DATAFREE_L2_WEIGHT:-0.0001}"
FEDCD_PM_TEACHER_DATAFREE_INIT_FROM_PROTO="${FEDCD_PM_TEACHER_DATAFREE_INIT_FROM_PROTO:-true}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ENABLE="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ENABLE:-false}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_NOISE_DIM="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_NOISE_DIM:-128}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_HIDDEN_DIM="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_HIDDEN_DIM:-512}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_LR="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_LR:-0.001}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_STEPS="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_STEPS:-1}"
FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ANCHOR_WEIGHT="${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ANCHOR_WEIGHT:-0.1}"
...
FEDCD_INIT_PRETRAIN="${FEDCD_INIT_PRETRAIN:-true}"
...
# Accuracy-first FedCD v2 defaults
FEDCD_ENTROPY_TEMP_PM="${FEDCD_ENTROPY_TEMP_PM:-1.0}"
FEDCD_ENTROPY_TEMP_GM="${FEDCD_ENTROPY_TEMP_GM:-1.0}"
FEDCD_ENTROPY_MIN_PM_WEIGHT="${FEDCD_ENTROPY_MIN_PM_WEIGHT:-0.35}"
FEDCD_ENTROPY_MAX_PM_WEIGHT="${FEDCD_ENTROPY_MAX_PM_WEIGHT:-0.75}"
FEDCD_ENTROPY_GATE_TAU="${FEDCD_ENTROPY_GATE_TAU:-0.20}"
FEDCD_ENTROPY_PM_BIAS="${FEDCD_ENTROPY_PM_BIAS:-0.0}"
...
FEDCD_ENTROPY_RELIABILITY_SCALE="${FEDCD_ENTROPY_RELIABILITY_SCALE:-0.4}"
...
FEDCD_FUSION_MODE="${FEDCD_FUSION_MODE:-class_aware_hard}"
...
FEDCD_PM_DEFER_CONF_THRESHOLD="${FEDCD_PM_DEFER_CONF_THRESHOLD:-0.35}"
FEDCD_PM_DEFER_GM_MARGIN="${FEDCD_PM_DEFER_GM_MARGIN:-0.10}"
FEDCD_PM_DEFER_OOD_THRESHOLD="${FEDCD_PM_DEFER_OOD_THRESHOLD:-0.00}"
FEDCD_CLASS_ROUTE_REL_THRESHOLD="${FEDCD_CLASS_ROUTE_REL_THRESHOLD:-0.55}"
FEDCD_CLASS_ROUTE_MARGIN="${FEDCD_CLASS_ROUTE_MARGIN:-0.02}"
FEDCD_CLASS_ROUTE_USE_OOD_GATE="${FEDCD_CLASS_ROUTE_USE_OOD_GATE:-false}"
...
FEDCD_TASK_FEXT_LR_SCALE="${FEDCD_TASK_FEXT_LR_SCALE:-1.0}"
FEDCD_CLUSTER_FEXT_EMA="${FEDCD_CLUSTER_FEXT_EMA:-0.99}"
FEDCD_RESIDUAL_MODULE_ENABLE="${FEDCD_RESIDUAL_MODULE_ENABLE:-true}"
FEDCD_RESIDUAL_HIDDEN_DIM="${FEDCD_RESIDUAL_HIDDEN_DIM:-128}"
FEDCD_FUSION_WEIGHT="${FEDCD_FUSION_WEIGHT:-}"
FEDCD_PM_LOGITS_WEIGHT="${FEDCD_PM_LOGITS_WEIGHT:-1.0}"
FEDCD_PM_ONLY_WEIGHT="${FEDCD_PM_ONLY_WEIGHT:-}"
FEDCD_GM_LOGITS_WEIGHT="${FEDCD_GM_LOGITS_WEIGHT:-1.0}"
FEDCD_BRANCH_CONSISTENCY_WEIGHT="${FEDCD_BRANCH_CONSISTENCY_WEIGHT:-}"
FEDCD_BRANCH_CONSISTENCY_TEMP="${FEDCD_BRANCH_CONSISTENCY_TEMP:-2.0}"
FEDCD_RESIDUAL_LOGIT_REG_WEIGHT="${FEDCD_RESIDUAL_LOGIT_REG_WEIGHT:-0.0001}"
FEDCD_PM_TO_GM_MASK_ENABLE="${FEDCD_PM_TO_GM_MASK_ENABLE:-false}"
FEDCD_PM_TO_GM_MASK_UNIFIED="${FEDCD_PM_TO_GM_MASK_UNIFIED:-true}"

case "${FEDCD_FUSION_MODE}" in
  class_aware_hard|pm_defer_hard|branch_select_hard|router_hard)
    FEDCD_FUSION_WEIGHT="${FEDCD_FUSION_WEIGHT:-0.0}"
    FEDCD_PM_ONLY_WEIGHT="${FEDCD_PM_ONLY_WEIGHT:-0.0}"
    FEDCD_BRANCH_CONSISTENCY_WEIGHT="${FEDCD_BRANCH_CONSISTENCY_WEIGHT:-0.0}"
    ;;
  *)
    FEDCD_FUSION_WEIGHT="${FEDCD_FUSION_WEIGHT:-1.0}"
    FEDCD_PM_ONLY_WEIGHT="${FEDCD_PM_ONLY_WEIGHT:-0.5}"
    FEDCD_BRANCH_CONSISTENCY_WEIGHT="${FEDCD_BRANCH_CONSISTENCY_WEIGHT:-0.1}"
    ;;
esac
...
echo "gm-update: mode=${FEDCD_GM_UPDATE_MODE} pm_to_gm_mask=${FEDCD_PM_TO_GM_MASK_ENABLE} unified_mask=${FEDCD_PM_TO_GM_MASK_UNIFIED}"
echo "fusion: mode=${FEDCD_FUSION_MODE} pm_conf=${FEDCD_PM_DEFER_CONF_THRESHOLD} gm_margin=${FEDCD_PM_DEFER_GM_MARGIN} ood=${FEDCD_PM_DEFER_OOD_THRESHOLD}"
echo "class-route: pm_rel=${FEDCD_CLASS_ROUTE_REL_THRESHOLD} margin=${FEDCD_CLASS_ROUTE_MARGIN} use_ood=${FEDCD_CLASS_ROUTE_USE_OOD_GATE}"
echo "loss: fusion=${FEDCD_FUSION_WEIGHT} gm=${FEDCD_GM_LOGITS_WEIGHT} pm=${FEDCD_PM_LOGITS_WEIGHT} pm_extra=${FEDCD_PM_ONLY_WEIGHT} consistency=${FEDCD_BRANCH_CONSISTENCY_WEIGHT}"
echo "teacher: source=${TEACHER_SOURCE}, topk=${TEACHER_TOPK}, min_active=${TEACHER_MIN_ACTIVE}"
echo "teacher-competence: enable=${FEDCD_PM_TEACHER_COMPETENCE_FILTER} rel=${FEDCD_PM_TEACHER_COMPETENCE_REL_WEIGHT} support=${FEDCD_PM_TEACHER_COMPETENCE_SUPPORT_WEIGHT} proto=${FEDCD_PM_TEACHER_COMPETENCE_PROTO_WEIGHT} min=${FEDCD_PM_TEACHER_COMPETENCE_MIN}"
...
CONDA_NO_PLUGINS=true conda run --no-capture-output -n "${CONDA_ENV}" \
  python -u system/main.py \
    ...
    --threshold_step_max "${THRESHOLD_STEP_MAX}" \
    --threshold_decay "${THRESHOLD_DECAY}" \
    --act_window_size "${ACT_WINDOW_SIZE}" \
    --act_min_slope "${ACT_MIN_SLOPE}" \
    ...
    --fedcd_task_fext_train_mode "${FEDCD_TASK_FEXT_TRAIN_MODE}" \
    --fedcd_task_fext_lr_scale "${FEDCD_TASK_FEXT_LR_SCALE}" \
    --fedcd_cluster_fext_ema "${FEDCD_CLUSTER_FEXT_EMA}" \
    --fedcd_residual_module_enable "${FEDCD_RESIDUAL_MODULE_ENABLE}" \
    --fedcd_residual_hidden_dim "${FEDCD_RESIDUAL_HIDDEN_DIM}" \
    --fedcd_branch_consistency_weight "${FEDCD_BRANCH_CONSISTENCY_WEIGHT}" \
    --fedcd_branch_consistency_temp "${FEDCD_BRANCH_CONSISTENCY_TEMP}" \
    --fedcd_residual_logit_reg_weight "${FEDCD_RESIDUAL_LOGIT_REG_WEIGHT}" \
    ...
    --fedcd_class_route_rel_threshold "${FEDCD_CLASS_ROUTE_REL_THRESHOLD}" \
    --fedcd_class_route_margin "${FEDCD_CLASS_ROUTE_MARGIN}" \
    --fedcd_class_route_use_ood_gate "${FEDCD_CLASS_ROUTE_USE_OOD_GATE}" \
    ...
    --fedcd_pm_teacher_competence_filter "${FEDCD_PM_TEACHER_COMPETENCE_FILTER}" \
    --fedcd_pm_teacher_competence_rel_weight "${FEDCD_PM_TEACHER_COMPETENCE_REL_WEIGHT}" \
    --fedcd_pm_teacher_competence_support_weight "${FEDCD_PM_TEACHER_COMPETENCE_SUPPORT_WEIGHT}" \
    --fedcd_pm_teacher_competence_proto_weight "${FEDCD_PM_TEACHER_COMPETENCE_PROTO_WEIGHT}" \
    --fedcd_pm_teacher_competence_min "${FEDCD_PM_TEACHER_COMPETENCE_MIN}" \
    --fedcd_pm_teacher_cluster_dist_weighting "${FEDCD_PM_TEACHER_CLUSTER_DIST_WEIGHTING}" \
    --fedcd_pm_teacher_cluster_dist_tau "${FEDCD_PM_TEACHER_CLUSTER_DIST_TAU}" \
    --fedcd_pm_teacher_cluster_dist_metric "${FEDCD_PM_TEACHER_CLUSTER_DIST_METRIC}" \
    --fedcd_pm_teacher_datafree_enable "${FEDCD_PM_TEACHER_DATAFREE_ENABLE}" \
    --fedcd_pm_teacher_datafree_batches "${FEDCD_PM_TEACHER_DATAFREE_BATCHES}" \
    --fedcd_pm_teacher_datafree_steps "${FEDCD_PM_TEACHER_DATAFREE_STEPS}" \
    --fedcd_pm_teacher_datafree_lr "${FEDCD_PM_TEACHER_DATAFREE_LR}" \
    --fedcd_pm_teacher_datafree_noise_scale "${FEDCD_PM_TEACHER_DATAFREE_NOISE_SCALE}" \
    --fedcd_pm_teacher_datafree_entropy_weight "${FEDCD_PM_TEACHER_DATAFREE_ENTROPY_WEIGHT}" \
    --fedcd_pm_teacher_datafree_diversity_weight "${FEDCD_PM_TEACHER_DATAFREE_DIVERSITY_WEIGHT}" \
    --fedcd_pm_teacher_datafree_l2_weight "${FEDCD_PM_TEACHER_DATAFREE_L2_WEIGHT}" \
    --fedcd_pm_teacher_datafree_init_from_proto "${FEDCD_PM_TEACHER_DATAFREE_INIT_FROM_PROTO}" \
    --fedcd_pm_teacher_datafree_generator_enable "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ENABLE}" \
    --fedcd_pm_teacher_datafree_generator_noise_dim "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_NOISE_DIM}" \
    --fedcd_pm_teacher_datafree_generator_hidden_dim "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_HIDDEN_DIM}" \
    --fedcd_pm_teacher_datafree_generator_lr "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_LR}" \
    --fedcd_pm_teacher_datafree_generator_steps "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_STEPS}" \
    --fedcd_pm_teacher_datafree_generator_anchor_weight "${FEDCD_PM_TEACHER_DATAFREE_GENERATOR_ANCHOR_WEIGHT}" \

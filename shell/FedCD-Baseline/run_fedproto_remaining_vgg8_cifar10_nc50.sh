...
QUEUE_ROOT="$SCRIPT_DIR/batch_runs/fedproto_remaining_cifar10_nc50/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
...
is_complete() {
    local partition="$1"
    local alpha="$2"
    local path="$SCRIPT_DIR/logs/cifar10/FedProto/GM_VGG8/$partition"
...
echo "[INFO] Running remaining FedProto for CIFAR-10 NC50"

for scenario in "${scenarios[@]}"; do
    if [[ "$scenario" == pat_* ]]; then
        partition="pat"
        alpha=""
    else
        partition="dir"
...
echo "[INFO] Finished remaining FedProto CIFAR-10 NC50 queue."

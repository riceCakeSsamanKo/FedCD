...
QUEUE_ROOT="$REPO_DIR/batch_runs/once_cifar100_dir05_nc20_ours/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
STATUS_CSV="$QUEUE_ROOT/status.csv"
echo "group,algorithm,seed,status,exit_code,start_utc,end_utc" > "$STATUS_CSV"
SEED="0"
start_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "[START] ours(t5) seed=${SEED} dataset=Cifar100_dir0.5_nc20"
/home/mulsoap0504/PFLlib-fresh/shell/run_fedccmv12_common_cifar100_nc20.sh Cifar100_dir0.5_nc20 \
  ...
  -go "once_FedCCMV12_t5_cifar100_dir05_nc20_seed${SEED}_${DATE_STR}_${TIME_STR}" \
  --seed "$SEED"
exit_code=$?
end_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
[[ $exit_code -eq 0 ]] && status="ok" || status="failed"
echo "ours,FedCCMV12_t5,${SEED},${status},${exit_code},${start_utc},${end_utc}" >> "$STATUS_CSV"
echo "[INFO] CIFAR-100 DIR(0.5) NC20 ours-once queue finished."

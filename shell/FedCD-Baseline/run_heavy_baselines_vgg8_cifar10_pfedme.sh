...
if [[ "${SKIP_PFEDME_BASELINES:-0}" == "1" || -f /tmp/skip_pfedme_baselines ]]; then
    echo "[INFO] SKIP_PFEDME_BASELINES is active; skipping CIFAR-10 pFedMe queue."
    exit 0
fi
...
QUEUE_ROOT="$SCRIPT_DIR/batch_runs/heavy_pfedme_cifar10/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
...
echo "[INFO] Running pFedMe for CIFAR-10"
...
echo "[INFO] Finished pFedMe CIFAR-10 queue."

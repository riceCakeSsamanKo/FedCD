...
QUEUE_ROOT="$REPO_DIR/batch_runs/fair_cifar100_dir05_nc50/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
STATUS_CSV="$QUEUE_ROOT/status.csv"
echo "group,algorithm,seed,status,exit_code,start_utc,end_utc" > "$STATUS_CSV"

DATASET="Cifar100_dir0.5_nc50"
MODEL="VGG8"
DEVICE="cuda"
DEVICE_ID="0"
GLOBAL_ROUNDS="100"
...
run_baseline() {
    ...
    local seed="$2"
    shift 2
    local goal="fair_${algo}_cifar100_dir05_nc50_seed${seed}_${DATE_STR}_${TIME_STR}"
...
run_ours() {
    ...
    /home/mulsoap0504/PFLlib-fresh/shell/run_fedccmv12_common_cifar100_nc50.sh Cifar100_dir0.5_nc50 \
      ...
      -go "fair_FedCCMV12_t5_cifar100_dir05_nc50_seed${seed}_${DATE_STR}_${TIME_STR}" \
      --seed "$seed"
    exit_code=$?
    end_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    [[ $exit_code -eq 0 ]] && status="ok" || status="failed"
...
echo "[INFO] Fair CIFAR-100 DIR(0.5) NC50 5-seed queue finished."

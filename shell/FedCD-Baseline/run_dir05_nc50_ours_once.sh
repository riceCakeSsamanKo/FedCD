...
QUEUE_ROOT="$REPO_DIR/batch_runs/once_dir05_nc50_ours/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
STATUS_CSV="$QUEUE_ROOT/status.csv"
echo "group,algorithm,seed,status,exit_code,start_utc,end_utc" > "$STATUS_CSV"
SEED="0"
start_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "[START] ours(t5) seed=${SEED} dataset=Cifar10_dir0.5_nc50"

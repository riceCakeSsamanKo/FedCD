...
WINDOW_NAME="${2:-VGG8-MNIST-NC50}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_SCRIPT="$SCRIPT_DIR/run_requested_baselines_vgg8_mnist_nc50.sh"
...
RUN_CMD="cd \"$SCRIPT_DIR\" && bash \"$RUN_SCRIPT\"; EXIT_CODE=\$?; echo; echo \"[EXIT] requested MNIST NC50 baseline queue finished with code \$EXIT_CODE\"; exec bash"

...
QUEUE_ROOT="$REPO_DIR/batch_runs/fair_dir05_nc20/date_${DATE_STR}/time_${TIME_STR}"
mkdir -p "$QUEUE_ROOT"
...
run_baseline() {
    ...
    local goal="fair_${algo}_dir05_nc20_seed${seed}_${DATE_STR}_${TIME_STR}"
    local start_utc
    local end_utc
    local exit_code=0
    ...
    if [[ $exit_code -eq 0 ]]; then
        status="ok"
        echo "[DONE] baseline algo=${algo} seed=${seed}"
    else
        status="failed"
        echo "[FAIL] baseline algo=${algo} seed=${seed} exit_code=${exit_code}"
    fi
...
run_ours() {
    ...
    /home/mulsoap0504/PFLlib-fresh/shell/run_fedccmv12_common_cifar10_nc20.sh Cifar10_dir0.5_nc20 \
      ...
      -go "fair_FedCCMV12_t5_dir05_nc20_seed${seed}_${DATE_STR}_${TIME_STR}" \
      --seed "$seed"
    exit_code=$?
    ...
    if [[ $exit_code -eq 0 ]]; then
        status="ok"
        echo "[DONE] ours(t5) seed=${seed}"
    else
        status="failed"
        echo "[FAIL] ours(t5) seed=${seed} exit_code=${exit_code}"
    fi
...
echo "[INFO] Fair DIR(0.5) NC20 5-seed queue finished."

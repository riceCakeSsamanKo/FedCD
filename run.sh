...
do
    ...
    do
        ...
        if [ -d "$LATEST_LOG_DIR" ]; then
            cp "dataset/$DATASET/config.json" "$LATEST_LOG_DIR/dataset_config_pat_NUM_CLUSTERS_$NUM_CLUSTERS_NUM_CLIENTS_$NUM_CLIENTS.json"
        ...
        LATEST_LOG_DIR=$(ls -td logs/exp_* | head -n 1)
        if [ -d "$LATEST_LOG_DIR" ]; then
            cp "dataset/$DATASET/config.json" "$LATEST_LOG_DIR/dataset_config_dir_NUM_CLUSTERS_$NUM_CLUSTERS_NUM_CLIENTS_$NUM_CLIENTS.json.json"
